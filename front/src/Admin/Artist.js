...
function Artist() {
  ...
  useEffect(() => {
    fetch(`${BASE_URL}/Registration/viewartist`)
      .then(res => res.json())
      .then(result => {
  ...
  const artistUpdate = (id) => {
    fetch(`${BASE_URL}/Registration/editartist`, {
      method: 'POST',
      headers: {
        Accept: 'application/json',
        'Content-Type': 'application/json'
      },
      body: JSON.stringify({ id, userstatus: 1 })
    })
    .then(res => res.json())
    .then(result => {
      if (result && result._id) {
        setArtists(artists.map(artist =>
          artist._id === id ? { ...artist, userid: { ...artist.userid, userstatus: 1 } } : artist
        ));
      navigate('/artist')
    } else {
                ...
                <tbody>
                  {artists.map((artist, index) => (
                    <tr key={artist._id}>
                      ...
                      <td>
                        <button type="button" className="btn btn-outline-success m-2" onClick={() => artistUpdate(artist.userid._id)}>
                          Approve
                        </button>
